"""Create and enqueue idempotent batch-plan tasks."""
...
TASK_TYPE_PREPARE_BATCHES = "PREPARE_BATCHES"


def enqueue_prepare_batches(request, datastore, scan_id, bulk_job_id):
    """Create and enqueue one idempotent PREPARE_BATCHES task."""
    ...
    if bulk_row.get("status") == "PROCESSING_PLANNED":
        return json_response(
            {
                "scanId": scan_id,
    ...
    if bulk_row.get("status") != "DOWNLOADED":
        return json_response(
            {
                "error": {
                    "code": "BULK_JOB_STATE_CONFLICT",
                    "message": (
                        "Processing cannot be prepared from state "
    ...
    task_key = canonical_hash(
        {
            "task_type": TASK_TYPE_PREPARE_BATCHES,
            "bulk_job_id": bulk_job_id,
            "source_file_id": bulk_row.get("source_file_id"),
            "source_file_checksum": bulk_row.get("source_file_checksum"),
            "batch_size": 5000,
            "version": "batch-plan-v1",
        }
    )
    ...
    # Catalyst restricts instant job names to 20 characters.
    ...
    try:
        ...
        with profile_stage(
            "orchestration",
            "enqueue_prepare_batches",
            scanId=scan_id,
            bulkJobId=bulk_job_id,
            taskId=task_row["task_id"],
        ):
            job_response = app.job_scheduling().job.submit_job(
                {
                    "job_name": job_name,
        ...
        LOGGER.exception(
            "Worker enqueue outcome unknown task_id=%s pool=%s target=%s error=%s",
            task_row["task_id"],
            pool_name,
            function_name,
            catalyst_error,
        ...
        if 400 <= status_code < 500:
            ...
            except Exception:  # noqa: BLE001 - preserve the original Catalyst error
                LOGGER.exception(
                    "Could not restore rejected task %s to PAUSED_RETRYABLE",
                    task_row["task_id"],
                )
            return json_response(
                {
        ...
        return json_response(
            {
                "error": {
                    "code": "WORKER_ENQUEUE_OUTCOME_UNKNOWN",
                    "message": (
                        "The Job Pool did not return a conclusive response. The task "
                        "remains QUEUED and must not be resubmitted automatically."
                    ),
                },
                "taskId": task_row["task_id"],
                "details": catalyst_error,
