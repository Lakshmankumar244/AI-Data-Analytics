...
from common.profiler import logger as profile_logger, profile_stage

logger = logging.getLogger(__name__)

BULK_STATUS_POLL_SECONDS = 5


def _status_profile(stage, **payload):
    profile_logger.info(
        "SCAN_PROFILE %s",
        json.dumps(
            {
...
def refresh_bulk_read_status(
        ...
        try:
            ...
            if retry_at > now_utc:
                ...
                _status_profile(
                    ...
                    scanId=scan_id,
                    bulkJobId=bulk_job_id,
                    nextRetryAt=to_catalyst_column_datetime(retry_at),
                    remainingSeconds=retry_after,
    ...
    if provider_state in {"ADDED", "QUEUED", "IN PROGRESS"}:
        next_retry_at = datetime.now(timezone.utc) + timedelta(
            seconds=BULK_STATUS_POLL_SECONDS
        )
        try:
        ...
        persisted_retry_at = to_catalyst_column_datetime(next_retry_at)
        _status_profile(
            "provider_processing",
            scanId=scan_id,
            bulkJobId=bulk_job_id,
            providerState=provider_state,
            nextRetryAt=persisted_retry_at,
            retryAfterSeconds=BULK_STATUS_POLL_SECONDS,
        )
        return json_response(
            {
                "scanId": scan_id,
